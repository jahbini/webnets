...
class TwitterOAuth {/*{{{*/
  ...
  function getRequestToken() {/*{{{*/
    ...
    if(!$r) {
	    Debug::show($this->responseHeaders() );
	    echo("<h1> ACK! </h1>");
    }
  ...
  function oAuthParseResponse($responseString) {
    ...
    Debug::show($responseString);
    Debug::show($r);
    Debug::show("Oauth Response");
    return $r;
  }
  ...
  error_log("oAuthrequest url=$url, method= $method| params=" . print_r($args,1) );
   ...
   /*
   * return any or all response headers from twitter
   */
   public function responseHeaders($head='') {
	   if ($head == '' )  { return $this->responseHeader;}
	   return $this->responseHeader[$head];
   }
  ...
  //error_log("http Curl url=$url|");
  //error_log("Post Data = " . print_r($post_data,1) );
    $ch = curl_init();
